      ...
      if currentPos == pos1 then
        if pos1 == pos2 then
          ...
          let advancement             = advanceLinesAndColumns pos1 pos2
              postFix                 = if advancesLines advancement then postFixLines else mempty
              (len, (lexeme, bsTail)) = spanAdvancementElm advancement bs
          in loopOverForElm bsTail (Pos l2 c2, result <> sepElmStart <> fromString (show len) <> fromString (":" ++ tname) <> postFix <> sepElmEnd <> lexeme) tokenTail
      else
        let advancement                 = advanceLinesAndColumns currentPos pos1
            postFix                     = if advancesLines advancement then postFixLines else mempty
            (len, (whitespace, bsTail)) = spanAdvancementElm advancement bs
        in loopOverForElm bsTail (Pos l1 c1, result <> sepElmStart <> fromString (show len) <> fromString ":WS" <> postFix <> sepElmEnd <> whitespace) tokens
